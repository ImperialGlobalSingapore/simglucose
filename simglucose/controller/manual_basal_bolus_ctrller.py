...
class BBController(Controller):
    ...
    def policy(self, observation, reward, done, **kwargs):
        ...
        give_bolus = kwargs.get('give_bolus', False)
        pname = kwargs.get('patient_name')
        meal = kwargs.get('meal')  # unit: g/min

        action = self._bb_policy(pname, meal, observation.CGM, sample_time, give_bolus)
        return action

    def _bb_policy(self, name, meal, glucose, env_sample_time, give_bolus=False):
        """

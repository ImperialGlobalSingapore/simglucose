...
class Qwen2TSModel(Qwen2Model):
    def __init__(self, config: Qwen2Config, ts_model):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        self.ts_embeddings = ts_model  # None  # will be set later
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, ts_model):
        super().__init__(config)
        self.model = Qwen2TSModel(config, ts_model=ts_model)
        self.vocab_size = config.vocab_size
